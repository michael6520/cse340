...
async function getModelByInventoryId(inv_id) {
  try {
    const data = await pool.query(
    ...
    console.error("getModelByInventoryId error " + error)
  }
}
...
async function insertInventory(vehicleData) {
  ...
  try {
    ...
    console.error("Error inserting inverntory: ", error)
    return false
  }
}
 ...
 * ************************** */
async function vaporizeCar(inv_id) {
  try {
    const sql =
